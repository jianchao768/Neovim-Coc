...
function parseAnsiHighlights(line, markdown = false) {
    ...
    for (let item of items) {
        ...
        let len = (0, string_1.byteLength)(newLabel);
        let span = [len, len + (0, string_1.byteLength)(item.text)];
        if (foreground && background) {
            let hlGroup = `CocList${(0, string_1.upperFirst)(foreground)}${(0, string_1.upperFirst)(background)}`;
            ...
            if (markdown) {
                if (foreground == 'yellow') {
                    ...
                    hlGroup = `CocListFg${(0, string_1.upperFirst)(foreground)}`;
                }
            }
            else {
                hlGroup = `CocListFg${(0, string_1.upperFirst)(foreground)}`;
            ...
            let hlGroup = `CocListBg${(0, string_1.upperFirst)(background)}`;
            highlights.push({ span, hlGroup });
        }
        if (item.bold) {
...
function ansiparse(str) {
    ...
    if (matchingText) {
        state.text = matchingText + (0, string_1.toText)(matchingControl);
        result.push(state);
    }
    return result;
}
