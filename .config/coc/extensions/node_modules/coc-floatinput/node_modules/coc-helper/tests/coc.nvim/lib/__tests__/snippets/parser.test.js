...
describe('SnippetParser', () => {
    test('transformEscapes', () => {
        assert.equal((0, parser_1.transformEscapes)('b\\uabc\\LDef'), 'bAbcdef');
        assert.equal((0, parser_1.transformEscapes)('b\\lAbc\\LDef'), 'babcdef');
        assert.equal((0, parser_1.transformEscapes)('b\\Uabc\\Edef'), 'bABCdef');
        assert.equal((0, parser_1.transformEscapes)('b\\LABC\\Edef'), 'babcdef');
        assert.equal((0, parser_1.transformEscapes)(' \\n \\t'), ' \n \t');
    });
    test('Empty Marker', () => {
        assert.ok(parser_1.Marker != null);
        assert.strictEqual((new parser_1.Text('')).snippet, undefined);
    });
    test('Empty CodeBlock', () => {
        let b = new parser_1.CodeBlock('', 'vim');
        b.update(new Map());
        b.resolve(undefined);
    ...
    test('Scanner', () => {
        ...
        assert.equal(scanner.next().type, 14 /* TokenType.EOF */);
        scanner.text('abc');
        assert.equal(scanner.next().type, 9 /* TokenType.VariableName */);
        assert.equal(scanner.next().type, 14 /* TokenType.EOF */);
        scanner.text('{{abc}}');
        assert.equal(scanner.next().type, 3 /* TokenType.CurlyOpen */);
        assert.equal(scanner.next().type, 3 /* TokenType.CurlyOpen */);
        assert.equal(scanner.next().type, 9 /* TokenType.VariableName */);
        assert.equal(scanner.next().type, 4 /* TokenType.CurlyClose */);
        assert.equal(scanner.next().type, 4 /* TokenType.CurlyClose */);
        assert.equal(scanner.next().type, 14 /* TokenType.EOF */);
        scanner.text('abc() ');
        assert.equal(scanner.next().type, 9 /* TokenType.VariableName */);
        assert.equal(scanner.next().type, 15 /* TokenType.OpenParen */);
        assert.equal(scanner.next().type, 16 /* TokenType.CloseParen */);
        assert.equal(scanner.next().type, 10 /* TokenType.Format */);
        assert.equal(scanner.next().type, 14 /* TokenType.EOF */);
        scanner.text('abc 123');
        assert.equal(scanner.next().type, 9 /* TokenType.VariableName */);
        assert.equal(scanner.next().type, 10 /* TokenType.Format */);
        assert.equal(scanner.next().type, 8 /* TokenType.Int */);
        assert.equal(scanner.next().type, 14 /* TokenType.EOF */);
        scanner.text('$foo');
        assert.equal(scanner.next().type, 0 /* TokenType.Dollar */);
        assert.equal(scanner.next().type, 9 /* TokenType.VariableName */);
        assert.equal(scanner.next().type, 14 /* TokenType.EOF */);
        ...
        assert.equal(scanner.next().type, 0 /* TokenType.Dollar */);
        assert.equal(scanner.next().type, 9 /* TokenType.VariableName */);
        assert.equal(scanner.next().type, 12 /* TokenType.Dash */);
        assert.equal(scanner.next().type, 9 /* TokenType.VariableName */);
        assert.equal(scanner.next().type, 14 /* TokenType.EOF */);
        scanner.text('${foo}');
        assert.equal(scanner.next().type, 0 /* TokenType.Dollar */);
        ...
        assert.equal(scanner.next().type, 3 /* TokenType.CurlyOpen */);
        assert.equal(scanner.next().type, 8 /* TokenType.Int */);
        assert.equal(scanner.next().type, 1 /* TokenType.Colon */);
        assert.equal(scanner.next().type, 9 /* TokenType.VariableName */);
        assert.equal(scanner.next().type, 4 /* TokenType.CurlyClose */);
        assert.equal(scanner.next().type, 14 /* TokenType.EOF */);
        scanner.text('\\${}');
        assert.equal(scanner.next().type, 5 /* TokenType.Backslash */);
        assert.equal(scanner.next().type, 0 /* TokenType.Dollar */);
        assert.equal(scanner.next().type, 3 /* TokenType.CurlyOpen */);
        assert.equal(scanner.next().type, 4 /* TokenType.CurlyClose */);
        ...
        assert.equal(scanner.next().type, 3 /* TokenType.CurlyOpen */);
        assert.equal(scanner.next().type, 9 /* TokenType.VariableName */);
        assert.equal(scanner.next().type, 6 /* TokenType.Forwardslash */);
        assert.equal(scanner.next().type, 9 /* TokenType.VariableName */);
        assert.equal(scanner.next().type, 6 /* TokenType.Forwardslash */);
        assert.equal(scanner.next().type, 9 /* TokenType.VariableName */);
        assert.equal(scanner.next().type, 6 /* TokenType.Forwardslash */);
    ...
    function assertText(value, expected, ultisnip = false) {
        const p = new parser_1.SnippetParser(ultisnip);
    ...
    test('Parser, escaped ultisnips', () => {
        const actual = new parser_1.SnippetParser(true).text('t\\`a\\`\n\\$ \\{\\}');
        expect(actual).toBe('t`a`\n$ {}');
    });
    test('Parser, transform with empty placeholder', () => {
        const actual = new parser_1.SnippetParser(true).text('${1} ${1/^(.*)/$1aa/}');
        expect(actual).toBe(' aa');
    });
    test('Parser, isPlainText()', function () {
        const s = (input, res) => {
            assert.equal(parser_1.SnippetParser.isPlainText(input), res);
        };
        s('abc', true);
        s('abc$0', true);
        s('ab$0chh', false);
    ...
    test('Parser, first placeholder / variable', function () {
        const first = (input) => {
            const p = new parser_1.SnippetParser(false);
            let s = p.parse(input, true);
            return s.first;
        };
        const assertPlaceholder = (m, index) => {
            assert.equal(m instanceof parser_1.Placeholder, true);
        ...
        let f = first('$foo $bar');
        assert.strictEqual(f instanceof parser_1.Variable, true);
    ...
    test('Parse, parse code block', () => {
        ...
        assertText('aa `!p\n  1\n  2`', 'aa ', true);
        const c = text => {
            return (new parser_1.SnippetParser(true)).parse(text);
        };
        assertMarker(c('`foo`'), parser_1.CodeBlock);
        assertMarker(c('`!v bar`'), parser_1.CodeBlock);
        assertMarker(c('`!p python`'), parser_1.CodeBlock);
        const assertPlaceholder = (text, kind, code) => {
            let p = c(text).children[0];
            assert.ok(p instanceof parser_1.Placeholder);
            let m = p.children[0];
            assert.ok(m instanceof parser_1.CodeBlock);
    ...
    test('Parser, placeholder with CodeBlock primary', () => {
        ...
        assert.ok(children[2] instanceof parser_1.CodeBlock);
    });
    ...
    test('Parser, hasCodeBlock()', () => {
        ...
        let s = (new parser_1.SnippetParser(false)).parse('`!p foo`', true);
    ...
    test('Parser, resolved variable', () => {
        ...
        let s = c('${1:${VISUAL}} $1');
        assert.ok(s.children[0] instanceof parser_1.Placeholder);
        assert.ok(s.children[0].children[0] instanceof parser_1.Variable);
        let v = s.children[0].children[0];
    ...
    test('Parser variable with code', () => {
        ...
        assert.ok(s.children[0] instanceof parser_1.Text);
        assert.ok(s.children[1] instanceof parser_1.CodeBlock);
    });
    test('Parser, transform condition if text', () => {
        const p = new parser_1.SnippetParser(true);
    ...
    test('Parser, transform with ascii option', () => {
        let p = new parser_1.SnippetParser();
        let snip = p.parse('${1:pêche}\n${1/.*/$0/a}');
        expect(snip.toString()).toBe('pêche\npeche');
        p = new parser_1.SnippetParser();
        snip = p.parse('${1/.*/$0/a}\n${1:pêche}');
    ...
    test('Parser, choice marker', () => {
        ...
        assert.ok(placeholders[0].choice.clone() instanceof parser_1.Choice);
    ...
    test('Parser, real world', () => {
        ...
        assert.ok(placeholder.children[1].clone() instanceof parser_1.Variable);
    ...
    test('Parser, ConditionString', () => {
        assert.ok(parser_1.ConditionString != undefined);
        let s = new parser_1.ConditionString(0, 'if', 'else');
        assert.strictEqual(s.toTextmateString(), '(?0:if:else)');
        s = new parser_1.ConditionString(0, 'if', '');
    ...
    test('Parser, FormatString', () => {
        let { children } = new parser_1.SnippetParser().parse('${foo/^x/complex${1:?if:else}/i}');
        let transform = children[0]['transform'];
        let res = transform.resolve('y');
        assert.strictEqual(res, 'complexelse');
        let formatString = transform.children[1];
        assert.strictEqual(formatString.toTextmateString(), '${1:?if:else}');
        let assertResolve = (shorthandName, value, result) => {
            let formatString = new parser_1.FormatString(0, shorthandName);
        ...
        let f = new parser_1.FormatString(0, undefined, 'if', undefined);
        assert.strictEqual(f.toTextmateString(), '${0:+if}');
        f = new parser_1.FormatString(0, undefined, undefined, 'else');
    ...
    test('TextmateSnippet#getTextBefore', () => {
        ...
        snippet = new parser_1.SnippetParser().parse('This ${1:foo ${2:is ${3:nested}}} $0', true);
    ...
    test('TextmateSnippet#deleteText', () => {
        let snippet = new parser_1.SnippetParser().parse('foo ${1:bar}', true);
    ...
    test('TextmateSnippet#insertSnippet with placeholder', () => {
        let snippet = new parser_1.SnippetParser().parse('${1:aaa} bbb ${2:ccc}}$0', true);
    ...
    test('TextmateSnippet#insertSnippet with variable', async () => {
        let snippet = new parser_1.SnippetParser().parse('|${foo} ${foo} ${u}|', true);
