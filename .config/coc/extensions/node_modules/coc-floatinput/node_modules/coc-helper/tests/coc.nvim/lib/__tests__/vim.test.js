...
describe('client API', () => {
    it('should set current dir', async () => {
        ...
        let res = await nvim.call('getcwd');
        expect((0, fs_1.sameFile)(res, __dirname)).toBe(true);
    });
    ...
    it('should set current buffer', async () => {
        let bufnr = await nvim.call('bufadd', ['foo']);
...
describe('Buffer API', () => {
    ...
    it('should add highlight', async () => {
        let ns = await nvim.createNamespace('test');
        await nvim.setLine('foo');
        let buf = await nvim.buffer;
...
describe('Popup', () => {
    it('should works for popup window', async () => {
        let winid = await nvim.call('popup_create', [['foo', 'bar'], {}]);
