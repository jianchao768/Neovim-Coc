...
class Mappings {
    constructor(manager, nvim) {
        ...
        this.addAction('do:help', async () => {
            var _a;
            await ((_a = manager.session) === null || _a === void 0 ? void 0 : _a.showHelp());
        });
        ...
        this.addAction('do:toggle', async () => {
            var _a;
            await ((_a = manager.session) === null || _a === void 0 ? void 0 : _a.ui.toggleSelection());
        });
        this.addAction('do:jumpback', () => {
            var _a;
            (_a = manager.session) === null || _a === void 0 ? void 0 : _a.jumpBack();
        });
    ...
    createMappings() {
        let insertMappings = configuration_1.default.get('insertMappings', {});
        this.userInsertMappings = this.fixUserMappings(insertMappings, 'list.insertMappings');
        let normalMappings = configuration_1.default.get('normalMappings', {});
    ...
    fixUserMappings(mappings, entry) {
        ...
        for (let [key, value] of Object.entries(mappings)) {
            if (!this.isValidAction(value)) {
                void window_1.default.showWarningMessage(`Invalid configuration - unable to support action "${value}" in "${entry}"`);
                continue;
            }
            if (key.length == 1) {
                res.set(key, value);
                ...
                if (key.toLowerCase() == '<space>') {
                    ...
                    if (!find)
                        void window_1.default.showWarningMessage(`Invalid configuration - unable to recognize "${key}" in "${entry}"`);
                }
            }
            else {
                void window_1.default.showWarningMessage(`Invalid configuration - unable to recognize key "${key}" in "${entry}"`);
            }
        }
        return res;
    }
    async navigate(up) {
        var _a;
        let ui = (_a = this.manager.session) === null || _a === void 0 ? void 0 : _a.ui;
        if (!ui)
            return false;
    ...
    async doInsertKeymap(key) {
        if (key === configuration_1.default.nextKey)
            return await this.navigate(false);
        if (key === configuration_1.default.previousKey)
