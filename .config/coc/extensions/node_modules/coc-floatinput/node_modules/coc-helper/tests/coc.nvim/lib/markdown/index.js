...
exports.parseMarkdown = exports.getHighlightItems = exports.parseDocuments = exports.toFiletype = void 0;
...
var FiletypeHighlights;
(function (FiletypeHighlights) {
    FiletypeHighlights["Error"] = "CocErrorFloat";
    FiletypeHighlights["Warning"] = "CocWarningFloat";
    FiletypeHighlights["Info"] = "CocInfoFloat";
    FiletypeHighlights["Hint"] = "CocHintFloat";
})(FiletypeHighlights || (FiletypeHighlights = {}));
const filetyepsMap = {
    js: 'javascript',
    ts: 'typescript',
    bash: 'sh'
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!inRange) {
            if (used + line.length > start) {
                inRange = true;
                let colStart = (0, string_1.byteIndex)(line, start - used);
                if (used + line.length > end) {
                    let colEnd = (0, string_1.byteIndex)(line, end - used);
                    inRange = false;
                    res.push({ colStart, colEnd, lnum: i + currline, hlGroup: ACTIVE_HL_GROUP });
                    break;
                }
                else {
                    let colEnd = (0, string_1.byteLength)(line);
                    res.push({ colStart, colEnd, lnum: i + currline, hlGroup: ACTIVE_HL_GROUP });
                }
            }
        }
...
function parseMarkdown(content, opts) {
    ...
    let filetype;
    let startLnum = 0;
    let parsed = (0, marked_1.marked)(content);
    ...
    for (let i = 0; i < parsedLines.length; i++) {
        ...
        if (opts.excludeImages && line.indexOf('![') !== -1) {
            ...
            if (!(0, node_1.stripAnsi)(line).trim().length)
                continue;
        }
        ...
        let res = (0, ansiparse_1.parseAnsiHighlights)(line, true);
        if (line === DIVIDE_LINE) {
            highlights.push({
                hlGroup: DIVIDING_LINE_HI_GROUP,
                lnum: currline,
