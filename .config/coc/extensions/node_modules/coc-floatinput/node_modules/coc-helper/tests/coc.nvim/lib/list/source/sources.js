...
class SourcesList extends basic_1.default {
    ...
    async loadItems() {
        ...
        return stats.map(stat => {
            ...
            return {
                label: `${prefix} ${(0, formatting_1.fixWidth)(stat.name, 22)} ${(0, formatting_1.fixWidth)('[' + stat.shortcut + ']', 10)} ${(0, formatting_1.fixWidth)(stat.triggerCharacters.join(''), 10)} ${(0, formatting_1.fixWidth)(stat.priority.toString(), 3)} ${stat.filetypes.join(',')}`,
                location,
                data: { name: stat.name }
            };
        });
