...
const buffer_1 = require("../../handler/refactor/buffer");
const changes_1 = tslib_1.__importDefault(require("../../handler/refactor/changes"));
...
function createEdit(uri) {
    let edit = vscode_languageserver_types_1.TextEdit.insert(vscode_languageserver_types_1.Position.create(0, 0), 'a');
    let doc = { uri, version: null };
    return { documentChanges: [vscode_languageserver_types_1.TextDocumentEdit.create(doc, [edit])] };
}
// assert ranges is expected.
async function assertSynchronized(buf) {
...
describe('fixChangeParams', () => {
    function createChangeParams(range, text, original, originalLines) {
        return {
            textDocument: {
                uri: 'untitled:/1',
                version: 1,
    ...
    it('should fix delete change params', async () => {
        let e = createChangeParams(vscode_languageserver_types_1.Range.create(0, 4, 2, 4), '', 'x\nfoo\n\u3000bar', [
            '\u3000barx',
            'foo',
            '\u3000bara'
        ]);
        e = (0, buffer_1.fixChangeParams)(e);
        expect(e.original).toBe('\u3000barx\nfoo\n');
        expect(e.contentChanges[0].range).toEqual(vscode_languageserver_types_1.Range.create(0, 0, 2, 0));
    });
    it('should fix insert change params', async () => {
        let e = createChangeParams(vscode_languageserver_types_1.Range.create(0, 4, 0, 4), 'x\nfoo\n\u3000bar', '', [
            '\u3000bara'
        ]);
        e = (0, buffer_1.fixChangeParams)(e);
        expect(e.original).toBe('');
        let change = e.contentChanges[0];
        expect(change.range).toEqual(vscode_languageserver_types_1.Range.create(0, 0, 0, 0));
...
describe('refactor', () => {
    ...
    describe('getFileRange()', () => {
        it('should throw when range does not exist', async () => {
            ...
            let locations = [{ uri, range: vscode_languageserver_types_1.Range.create(0, 0, 0, 6) }];
        ...
        it('should find file range', async () => {
            let uri = vscode_uri_1.URI.file(__filename).toString();
            let locations = [{ uri, range: vscode_languageserver_types_1.Range.create(0, 0, 0, 6) }];
            let buf = await commands_1.default.executeCommand('editor.action.showRefactor', locations);
    ...
    describe('getRange()', () => {
        it('should get delete range', async () => {
            let filename = await (0, helper_1.createTmpFile)('foo\n\nbar\n');
            let fileItem = {
                filepath: filename,
            ...
            expect(r).toEqual(vscode_languageserver_types_1.Range.create(3, 0, 6, 0));
            res = buf.getFileRange(7);
            r = buf.getDeleteRange(res);
            expect(r).toEqual(vscode_languageserver_types_1.Range.create(6, 0, 8, 0));
        });
        it('should get replace range', async () => {
            ...
            expect(r).toEqual(vscode_languageserver_types_1.Range.create(4, 0, 4, 3));
            res = buf.getFileRange(7);
            r = buf.getReplaceRange(res);
            expect(r).toEqual(vscode_languageserver_types_1.Range.create(7, 0, 7, 3));
        });
    });
    describe('fromWorkspaceEdit()', () => {
        ...
        it('should create from document changes', async () => {
            let edit = createEdit(vscode_uri_1.URI.file(__filename).toString());
        ...
        it('should create from workspaceEdit', async () => {
            let changes = {
                [vscode_uri_1.URI.file(__filename).toString()]: [{
                        ...
                        range: vscode_languageserver_types_1.Range.create(50, 0, 50, 1),
                        newText: ' '
                    }, {
                        range: vscode_languageserver_types_1.Range.create(60, 0, 60, 1),
                        newText: ' '
                    }]
            };
            let edit = { changes };
    ...
    describe('onChange()', () => {
        async function setup() {
            ...
            let locations = [{
                    ...
                    uri,
                    range: vscode_languageserver_types_1.Range.create(10, 0, 10, 6),
                }];
    ...
    describe('onDocumentChange()', () => {
        it('should ignore when change after range', async () => {
            ...
            await doc.synchronize();
            let buf = await refactor.fromLocations([{ uri: doc.uri, range: vscode_languageserver_types_1.Range.create(0, 0, 0, 3) }]);
        ...
        it('should adjust when change before range', async () => {
            ...
            await doc.buffer.append(['', '', '', '', 'foo', 'bar']);
            await doc.synchronize();
            let buf = await refactor.fromLocations([{ uri: doc.uri, range: vscode_languageserver_types_1.Range.create(4, 0, 4, 3) }]);
...
\u3000`;
    ...
    describe('createRefactorBuffer()', () => {
        ...
        it('should use conceal for line numbers', async () => {
            ...
            await buf.addFileItems([fileItem]);
            let arr = await nvim.call('getmatches');
    ...
    describe('saveRefactor()', () => {
        it('should adjust line ranges after change', async () => {
            ...
            let doc = workspace_1.default.getDocument(buf.bufnr);
            await doc.synchronize();
            let res = await helper_1.default.doAction('saveRefactor', doc.bufnr);
    ...
    describe('doRefactor', () => {
        ...
        it('should show message when prepare failed', async () => {
            await helper_1.default.createDocument();
            disposable = languages_1.default.registerRenameProvider(['*'], {
                prepareRename: () => {
                    return undefined;
                },
                provideRenameEdits: () => {
            ...
            await helper_1.default.doAction('refactor');
            let res = await helper_1.default.getCmdline();
        ...
        it('should open refactor window when edits is valid', async () => {
            ...
            disposable = languages_1.default.registerRenameProvider(['*'], {
                provideRenameEdits: () => {
                    let changes = {
                        [vscode_uri_1.URI.file(filepath).toString()]: [{
                                range: vscode_languageserver_types_1.Range.create(0, 0, 0, 6),
                                newText: ''
                            }, {
                                range: vscode_languageserver_types_1.Range.create(1, 0, 1, 6),
            ...
            await helper_1.default.createDocument(filepath);
            let winid = await nvim.call('win_getid');
            await refactor.doRefactor();
