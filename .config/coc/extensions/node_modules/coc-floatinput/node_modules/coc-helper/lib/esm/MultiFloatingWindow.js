...
export class MultiFloatingWindow {
    static async create(options) {
        const borderWin = await FloatingWindow.create(Object.assign({ hasBorderBuf: false, mode: 'show' }, options));
        const wins = await Promise.all(Object.entries(options.wins).map(async ([key, win]) => [
            key,
            await FloatingWindow.create(Object.assign({ hasBorderBuf: false }, win)),
        ]));
    ...
    constructor(borderFloatWin, floatWinDict, util) {
        ...
        this.disposables.push(events.on('BufWinLeave', async (bufnr) => {
