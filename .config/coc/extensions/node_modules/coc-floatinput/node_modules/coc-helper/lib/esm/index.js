...
export async function activateHelper(context, options = {}) {
    var _a;
    if ((_a = options.vimModule) !== null && _a !== void 0 ? _a : true) {
        await VimModule.init(context);
    }
    try {
        await workspace.nvim.command('hi default link CocHelperNormalFloatNC CocHelperNormalFloat');
    }
    catch (error) {
...
export async function activate(context) {
    ...
    await helperLogger.measureTask(async () => {
        await workspace.nvim.command('hi CocHelperNormalFloat ctermbg=Red guibg=#aa0000');
        await workspace.nvim.command('hi CocHelperNormalFloatNC ctermbg=Red guibg=#aa0000');
        await workspace.nvim.command('hi CocHelperNormalFloatBorder ctermbg=Black ctermfg=Grey guibg=#0000aa guifg=#ffffff');
    }, 'highlight commands');
    await helperLogger.measureTask(async () => {
        const floatWin = await FloatingWindow.create({
            mode: 'base',
        });
        commands.registerCommand('testHelper-floating', helperLogger.asyncCatch(async () => {
            const text = 'hello';
            await floatWin.open({
    ...
    await helperLogger.measureTask(async () => {
        ...
        commands.registerCommand('testHelper-multi-floating', helperLogger.asyncCatch(async () => {
            const width = 10;
