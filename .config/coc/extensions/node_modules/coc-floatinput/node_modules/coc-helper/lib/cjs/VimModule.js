...
exports.VimModule = void 0;
const coc_nvim_1 = require("coc.nvim");
const notifier_1 = require("./notifier");
const util_1 = require("./util");
const module_1 = require("./util/module");
const mid = (0, module_1.getModuleId)('VimModule');
const globalKey = `coc_helper_module_m${mid}_v${util_1.versionName}`;
const globalVariable = `g:${globalKey}`;
const callFunc = `CocHelperCallFn_m${mid}_v${util_1.versionName}`;
const declareVar = `CocHelperCallVar_m${mid}_v${util_1.versionName}`;
...
class VimModule {
    static async init(context) {
        this.inited = true;
        await coc_nvim_1.workspace.nvim.call('execute', `
      ...
      `);
        ...
        while (queue.length) {
            ...
            try {
                ...
                util_1.helperLogger.error(error);
            }
    ...
    static create(moduleName, cb) {
        const id = (0, module_1.getModuleId)('VimModule.module');
    ...
    fn(fnName, getContent) {
        ...
        this.registerInit(`fn ${name}`, async () => {
            util_1.helperLogger.debug(`declare fn ${name}`);
        ...
        return {
            ...
            call: (...args) => {
                util_1.helperLogger.debug(`call ${name}`);
            ...
            callNotify: (...args) => {
                util_1.helperLogger.debug(`callNotify ${name}`);
                return nvim.call(callFunc, [this.moduleKey, fnName, args], true);
            },
            callNotifier: (...args) => {
                util_1.helperLogger.debug(`callNotifier ${name}`);
    ...
    var(varName, expression) {
        ...
        this.registerInit(`var ${name}`, async () => {
            util_1.helperLogger.debug(`declare var ${name}`);
